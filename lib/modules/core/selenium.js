...
module.exports = function (grunt, config, modules) {
    ...
    grunt.template.portOption = function ()
    {
    ...
    config.exec.selenium_start      = {cmd: 'PORT=<%= grunt.template.portOption() %> selenium start && sleep 5s'};
    config.exec.selenium_stop       = {cmd: 'PORT=<%= grunt.template.portOption() %> selenium stop'};
    ...
    config.exec.selenium_test = {
        cmd: config.exec.selenium_lock_app.cmd + ';'
           + '<%= selenium.test.env %> '
           + 'URI=<%= grunt.template.uriOption() %> BROWSER=<%= grunt.task.current.args[0] %> '
           + 'TYPE=uat <%= utils.php %>/vendor/bin/phpunit -c tests/selenium '
    ...
    config.tasks['selenium'] = {
        ...
        function: function () {
            ...
            var port    = grunt.config.process('<%= grunt.template.portOption() %>');
