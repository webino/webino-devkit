...
module.exports = function (grunt, config, modules) {
    ...
    config.exec.selenium_daemon_start = {cmd: 'mkdir -p /tmp/selenium/daemon && touch /tmp/selenium/daemon/<%= grunt.template.seleniumPortOption() %>-selenium.start && sleep 5s'};
    ...
    config.onFail.enq({priority: 1, callback: function (e) {
        if (-1 !== e.indexOf('exec:selenium_test')) {
            ...
            exec(config.exec['stop-server'].check, {silent: true}).output.trim()
                && tasks.push('exec:stop-server');

            tasks.push('fail:Selenium failed!');
    ...
    config.tasks['selenium'] = {
        ...
        function: function () {
            ...
            var tasks   = [config.userCfg.selenium.daemon ? 'exec:selenium_daemon_start' : 'exec:selenium_start'];
