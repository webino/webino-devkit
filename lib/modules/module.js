...
exports.config = (function () {
    function config(grunt, config) {
        ...
        config.phpunit = {
            options: {
                configuration: 'tests',
                bin: '<%= utils.php %>/vendor/bin/phpunit',
        ...
        config.exec.tester = {cmd: '<%= utils.php %>/vendor/bin/tester -p php-cgi -c tests/tester/php.ini --coverage ._log/clover.xml --coverage-src src/ tests/tester/<%= grunt.template.testerJunit() %>'};

        grunt.template.testerJunit = function () {
            if (config.isBuild === true) {
                return ' -o junit > ._log/junit.xml';
            }
        };
