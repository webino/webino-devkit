module.exports = function config(grunt, config) {
    ...
    config.exec.tester    = {cmd: 'TYPE=unit <%= utils.php %>/vendor/bin/tester -p php-cgi -c <%= utils.php %>/tester/php.ini --coverage ._log/clover.xml --coverage-src src/ tests/tester/<%= grunt.template.tester() %>'};
