...
exports.config = (function () {
    function config(grunt, config) {
        ...
        config.phpunit = {
            options: {
                configuration: 'tests',
                bin: 'php -dzend_extension=xdebug.so -dmax_execution_time=1200 <%= utils.php %>/vendor/bin/phpunit',
        ...
        config.exec.tester    = {cmd: '<%= utils.php %>/vendor/bin/tester -p php-cgi -d zend_extension=xdebug.so -d max_execution_time=1200 --coverage ._log/clover.xml --coverage-src src/ tests/tester/<%= grunt.template.tester() %>'};
