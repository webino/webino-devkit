...
exports.config = (function () {
    function config(grunt, config) {
        ...
        config.clean.test_app_assets = {src: [
            '<%= test_app_path %>/public/assets/<%= pkg.dashCase %>/**/*.*',
            '<%= test_app_path %>/public/assets/<%= pkg.dashCase.replace(/^webino-/g, "") %>/**/*.*'
        ]};
        ...
        config.mkdir.test_app_tmp = {options: {create: ['<%= test_app_path %>/tmp/common']}};
        ...
        config.phpunit = {
            options: {
                configuration: 'tests',
                bin: 'TEST=unit php -dzend_extension=xdebug.so -dmax_execution_time=1200 <%= utils.php %>/vendor/bin/phpunit',
        ...
        config.exec.tester    = {cmd: 'TEST=unit <%= utils.php %>/vendor/bin/tester -p php-cgi -d zend_extension=xdebug.so -d extension=mailparse.so -d max_execution_time=1200 --coverage ._log/clover.xml --coverage-src src/ tests/tester/<%= grunt.template.tester() %>'};
        ...
        config.exec.test_app_make  = {cmd: 'git clone <%= test_app_vendor_git %>/<%= test_app_name %>.git --branch <%= test_app_git_branch %> --depth 1 <%= test_app_path %>;<%= test_app_git_extra %>'};
