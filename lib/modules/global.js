        ...
        function config(grunt, config) {
            ...
            config.tasks.self_setup = {
                ...
                function: function() {
                    ...
                    var userConfig = config.home + '/.webino.json';
                    if (grunt.file.exists(userConfig)) {
                        grunt.log.ok('Installing Webino user config ...');
                        grunt.util.spawn({cmd: 'cp', args: [__dirname + '/../../dist/webino.json', config.home + '/.webino.json']});
                    } else {
                        grunt.log.ok('Webino user config already installed...');
                    }
                }
            };
        };
