...
(function() {
    exports.config = config = (function() {
        function config(grunt, config) {
            ...
            config.mkdir.varlib = {
                options: {create: ['<%= varlib.root %>']}
            };

            /**
             * Waiting for varlib initialization
             */
            config.wait_async.varlib = {
                options: {
                    wait: function (done) {
                        var func = function () {
                            if (grunt.file.isDir(config.varlib.root)) {
                                grunt.task.run(['copy:varlib']);
                                done();
                            } else {
                                setTimeout(func, 250);
                            }
            ...
            /**
             * Waiting for varlib copying
             */
            config.wait_async.self_setup = {
                options: {
                    wait: function (done) {
                        var func = function () {
                            if (grunt.file.isDir(config.varlib.php)) {
                                var root = config.varlib.php;
                                process.chdir(root);

                                if (config.varlib.isInit()) {
            ...
            config.tasks.workspace = {
                ...
                function: function () {
                    ...
                    // crete user config
                    if (!grunt.file.exists(this.user_cfg)) {
                        /**
                         * Required to run as user
                         */
                        grunt.log.ok('Installing Webino user config ...');
                        grunt.util.spawn({cmd: 'cp', args: [__dirname + '/../dist/webino.json', this.user_cfg]}, function () {});
                    }
            ...
            config.tasks.self_setup = {
                ...
                function: function () {
                    if (config.varlib.isInit()) {
                        ...
                        grunt.task.run(['mkdir:varlib', 'wait_async:varlib']);
                    }

                    grunt.task.run('wait_async:self_setup');
                }
            };
        };
        return config;
