...
exports.config = function config(grunt, modules) {
    ...
    this.prompt  = require(__dirname + '/prompt.js')(grunt);
    ...
    if (grunt.file.exists(this.pkgCfgPath)) {
        ...
        /**
         * Returns true when the package uses PSR-4 autoloading
         *
         * @returns {boolean}
         */
        this.pkg.isPSR4 = function () {
            return 'undefined' !== typeof self.pkg.autoload['psr-4']
        };

        grunt.template.psr4Namespace = function () {
            return self.pkg.isPSR4() ? '' : '/<%= pkg.camelCase %>';
        };
    }
    ...
    /**
     * Setup additional devkit scripts
     */
    if (this.userCfg.scripts) {
        this.verbose && grunt.log.writeln('=> loading devkit additional scripts:');

        self.userCfg.scripts.forEach(function (path) {
            self.verbose && grunt.log.writeln(path);

            try {
                require(path)(grunt, self, modules);
            } catch(exc) {}
        });
    }
