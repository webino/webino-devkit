exports.config = function config(grunt, modules) {
    ...
    // crete user config
    if (process.getuid() && !grunt.file.exists(this.user_cfg)) {
        /**
         * Required to run as user
         */
        grunt.log.ok('Installing Webino user config ...');
        grunt.util.spawn({cmd: 'cp', args: [__dirname + '/../dist/webino.json', this.home + '/.webino.json']});
    }

    // read user config
    this.config = grunt.file.readJSON(this.user_cfg);
    ...
    this.setupDevkitModules = function (pathname) {
        ...
        files.forEach(function(filepath) {
            ...
            self.module_paths[key] = filepath;
        });
    };
    ...
    if (this.config.modules) {
        ...
        for (var i = 0; i < this.config.modules.length; i++) {
            (function () {
                ...
                return require(self.config.modules[i]);
